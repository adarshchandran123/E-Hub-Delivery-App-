...
export default function DlvryBoyEmailVerify() {
    ...
    const handleClick = () => {
        var email = localStorage.getItem('dlvryBoyDetails')
        ...
        try{
            ...
            axios.post("http://localhost:5000/dlvryBoy/dlvryBoyResendOTP",{email},config).then((response)=>{
    ...
    const handleSubmit = async () => {
        let Email = localStorage.getItem("dlvryBoyDetails")
            Email = JSON.parse(Email)
        var Email_And_OTP = {}
        ...
        try {
            ...
            axios.post('http://localhost:5000/dlvryBoy/dlvryBoyEmailVerify',Email_And_OTP, { config }).then((response) => {
                
                    localStorage.removeItem('dlvryBoyDetails')
                    navigate('/login')
            }).catch((err)=>{
