...
export default function ShopEmailVerify() {
    ...
    const handleClick = () => {
        var email = localStorage.getItem('shopDetails')
        ...
        try{
            ...
            axios.post("http://localhost:5000/shop/ShopResendOTP",{email},config).then((response)=>{
    ...
    const handleSubmit = async () => {
        let Email = localStorage.getItem("shopDetails")
            Email = JSON.parse(Email)
        var Email_And_OTP = {}
        ...
        try {
            ...
            axios.post('http://localhost:5000/shop/shop_Email_verify',Email_And_OTP, { config }).then((response) => {
